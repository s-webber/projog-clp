...
public class NorTest extends AbstractReificationTest {
   public NorTest() {
      super(Nor::new);

      given("1", "1").enforce(FAILED).prevent(MATCHED).reify(FAILED);
      given("0", "0").enforce(MATCHED).prevent(FAILED).reify(MATCHED);
      given("1", "0:1").enforce(FAILED).prevent(MATCHED).reify(FAILED);
      given("0:1", "1").enforce(FAILED).prevent(MATCHED).reify(FAILED);
      ...
      given("0:1", "0:1").enforce(MATCHED, "0").prevent(UNRESOLVED).reify(UNRESOLVED);
