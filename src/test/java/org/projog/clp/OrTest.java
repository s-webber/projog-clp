...
public class OrTest extends AbstractReificationTest {
   public OrTest() {
      super(Or::new);

      given("1", "1").enforce(MATCHED).prevent(FAILED).reify(MATCHED);
      given("0", "0").enforce(FAILED).prevent(MATCHED).reify(FAILED);
      given("1", "0:1").enforce(MATCHED).prevent(FAILED).reify(MATCHED);
      ...
      given("0:1", "0:1").enforce(UNRESOLVED).prevent(MATCHED, "0").reify(UNRESOLVED);
